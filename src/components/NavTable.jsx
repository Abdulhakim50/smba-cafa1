...
const NavTable = ({ data, selectedRow, setSelectedRow,isOpen, isOpenPrice ,selectedData, activeAdisyon}) => {
  return (
    ...
    <div className={`max-w-[400px]     border ${isOpen && 'hidden'}  `}>
      <h1 className='text-center pt-16 pb-3  font-semibold'>{isOpenPrice ? 'PRODUCTS' : 'REPORTS'}</h1> 
     {
...
{ activeAdisyon && activeAdisyonData.map((row) => (
       <div className={classNames('  cursor-pointer flex flex-col pad bg-[#FCFCFC]', {
        'bg-[#F0F3FA]': selectedRow === row.id,
      })}>
          <div
